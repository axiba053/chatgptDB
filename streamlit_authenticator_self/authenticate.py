...
class Authenticate:
    ...
    def reset_password(self, username: str, form_name: str, location: str='main') -> bool:
        ...
        self.email=reset_password_form.text_input('注册邮箱')
        self.password = reset_password_form.text_input('当前密码', type='password')
        new_password = reset_password_form.text_input('新密码', type='password')
        new_password_repeat = reset_password_form.text_input('确认新密码', type='password')
        if reset_password_form.form_submit_button('修改'):
            if self._check_credentials(inplace=False) and self.credentials['usernames'][self.username]['email']==self.email:
                if len(new_password) > 0:
                    if new_password == new_password_repeat:
                        if self.password != new_password: 
                            ...
                            raise ResetError('新密码不可和旧密码相同！')
                    else:
                        raise ResetError('新密码输入不一致！')
                else:
                    raise ResetError('请输入新密码！')
            else:
    ...
    def register_user(self, form_name: str, location: str='main', preauthorization=True) -> bool:
        ...
        new_email = register_user_form.text_input('邮箱')
        new_username = register_user_form.text_input('用户名').lower()
        new_name = register_user_form.text_input('昵称')
        new_password = register_user_form.text_input('密码', type='password')
        new_password_repeat = register_user_form.text_input('确认密码', type='password')

        if register_user_form.form_submit_button('注册'):
